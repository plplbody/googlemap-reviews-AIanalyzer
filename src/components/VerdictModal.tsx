        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white w-full max-w-4xl max-h-[90vh] rounded-3xl shadow-2xl relative flex flex-col overflow-hidden animate-in zoom-in-95 duration-300">
                ...
                <div className="overflow-y-auto flex-1 scrollbar-thin scrollbar-thumb-gray-200">
                    ...
                    <div className="bg-gradient-to-br from-orange-50 to-amber-50 pt-8 text-center border-b border-brand/10 relative overflow-hidden shrink-0">
                        <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-brand to-amber-500" />

                        <div className="inline-flex items-center px-4 py-1.5 bg-brand text-white rounded-full text-sm font-bold mb-4 shadow-sm border border-brand/20">
                            <Trophy size={14} className="fill-brand text-brand" />
                            あなたとのマッチ度 No.1
                        </div>

                        <h2 className="text-3xl md:text-4xl font-black text-brand-black mb-2 flex flex-col md:flex-row items-center justify-center gap-3">
                            <Crown className="w-10 h-10 text-brand fill-brand drop-shadow-sm" />
                            <span className="border-b-4 border-brand/20 decoration-none">{winner.name}</span>
                        </h2>

                        <div className="max-w-2xl mx-auto mt-6 relative">
                            <p className="text-lg text-brand font-bold leading-relaxed">
                                {verdict.reason}
                            </p>
                        </div>

                        <p className="text-xs text-brand-black/80 text-center my-3">
                            💡 評価(Good/Bad)でAI精度UP
                        </p>
                    </div>
                                ...
                                <table className="w-full min-w-[600px] border-collapse">
                                    <thead>
                                        <tr>
                                            ...
                                            {sortedPlaces.map(place => (
                                                <th key={place.id} className={`p-4 text-left font-bold text-gray-800 border-b border-gray-100 w-1/${selectedPlaces.length} min-w-[200px] ${place.id === verdict.winnerId ? 'bg-brand/5' : ''}`}>
                                                    <div className="flex items-center gap-2">
                                                        {place.id === verdict.winnerId && <Crown size={16} className="text-brand fill-brand" />}
                                                        {place.name}
                                                    </div>
                                                </th>
                                            ))}
                                    ...
                                    <tbody className="divide-y divide-gray-100">
                                        ...
                                        <tr>
                                            <td className="p-4 align-middle bg-gray-50/50 border-r border-gray-100">
                                                <div className="flex flex-col gap-1">
                                                    <span className="text-sm font-bold text-brand">マッチ度</span>
                                                    <div className="p-1.5 rounded-lg bg-white w-fit shadow-sm text-brand">
                                                        <Scale size={18} />
                                                    </div>
                                                </div>
                                            </td>
                                                ...
                                                return (
                                                    <td key={place.id} className={`p-4 align-middle ${isWinner ? 'bg-orange-50/30' : ''}`}>
                                                        <div className="flex items-baseline gap-1">
                                                            <span className={`text-3xl font-black tabular-nums ${isWinner ? 'text-brand' : 'text-brand-black/80'}`}>
                                                                {score}
                                                            </span>
                                                            <span className="text-sm font-bold text-brand-black/50">%</span>
                                                        </div>
                                                    </td>
                                                );
                                            })}
                                        ...
                                        {[
                                            ...
                                            <tr key={row.k} className="group hover:bg-brand-gray/20 transition-colors">
                                                <td className="p-4 align-top bg-gray-50/50 border-r border-gray-100 group-hover:brand-gray/20 transition-colors">
                                                    <div className="flex flex-col gap-1">
                                                    ...
                                                    return (
                                                        <td key={place.id} className={`p-4 align-top text-sm text-gray-700 leading-relaxed ${place.id === verdict.winnerId ? 'bg-orange-50/30' : ''}`}>
                                                            <div className="flex flex-col gap-3">
                                                                {/* Score Badge */}
                                                                <div className="text-2xl font-black text-brand-black tabular-nums">
                                                                    {score}
                                                                    <span className="text-base font-normal text-brand-black/50 ml-1">/5.0</span>
                                                                </div>
