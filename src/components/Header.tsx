    ...
    return (
        <nav className={`absolute top-0 w-full z-50 p-6 flex justify-between items-center transition-colors duration-300 ${viewState === 'HOME' ? 'text-white' : 'text-brand-black-dark'}`}>
            ...
            <div className="hidden md:flex gap-6 text-sm font-medium tracking-wide items-center">
                <Link href="/about" className="cursor-pointer hover:text-brand-orange-dark transition-colors">
                    ABOUT
                </Link>
                ...
                {user ? (
                    <div className="relative">
                        ...
                        >
                            {user.photoURL ? (
                                ...
                                <div className="w-9 h-9 rounded-full brand-gray flex items-center justify-center text-brand-black">
                        ...
                        {isProfileMenuOpen && (
                            <div className="absolute top-full right-0 mt-3 w-48 bg-white rounded-xl shadow-xl py-2 animate-in fade-in zoom-in-95 duration-200 border border-brand-gray overflow-hidden z-20">
                                <div className="px-4 py-3 border-b border-brand-gray-light">
                                    <p className="text-xs text-brand-black-light mb-1">Signed in as</p>
                    ...
                    <button
                        ...
                        className="cursor-pointer hover:text-brand-orange-dark transition-colors font-bold flex items-center gap-2 ml-4"
                    >
                        LOGIN
                    </button>
                )}
            ...
            {isMobileMenuOpen && (
                <div className="absolute top-full left-0 w-full bg-white/95 backdrop-blur-md shadow-lg p-6 flex flex-col gap-6 md:hidden animate-in slide-in-from-top-2 duration-200 border-t border-brand-gray">
                    <Link href="/about" className="text-brand-black-dark font-bold tracking-wider cursor-pointer hover:text-brand-orange-dark" onClick={() => setIsMobileMenuOpen(false)}>
                        ABOUT
                    </Link>
                    {user ? (
                        <div className="flex flex-col gap-4 border-t pt-4 border-brand-gray">
                            <div className="flex items-center gap-3 mb-2">
                                ...
                                <div>
                                    <p className="text-sm font-bold text-brand-black-dark">{user.displayName}</p>
                                    <p className="text-xs text-brand-black-light">{user.email}</p>
                                </div>
                            </div>
                        ...
                        <span
                            className="text-brand-black-dark font-bold tracking-wider cursor-pointer hover:text-brand-orange-dark"
