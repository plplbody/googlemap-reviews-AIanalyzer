...
| **U-09** | **Scene Picker** | <span style="color:red">Pending</span> | `src/components/ScenePicker.tsx` | **Trigger**: 評価(Good)後<br>**Process**: デフォルト利用シーン/カスタム利用シーンの選択、カスタム利用シーンの作成を行う。<br>**Output**: シーン選択ダイアログ。 |

### Logic (L-xx)
| ID | 名前 | Status | File / Method | 詳細 (Trigger + IPO) |
| :--- | :--- | :--- | :--- | :--- |
| **L-01** | **Search Process** | **Implemented** | `src/server/actions/place.ts`<br>`searchPlaces` | **Trigger**: 検索実行<br>**Input**: `query` (String)<br>**Process**: コスト最適化フロー。<br>1. TextSearch(New)で20件一括取得。<br>2. DBキャッシュ照合(有効期限＆分析済確認)。<br>3. 新規・期限切れのみ分析キューへ追加。<br>**Output**: `PlaceSearchResult[]` |
...
| **L-03** | **Re-ranking** | **Implemented** | `src/server/actions/personalize.ts` (Controller)<br>`src/server/services/scoring.ts` (Logic) | **Trigger**: リスト表示・更新時<br>**Input**: `placeIds`, `uid`, `scenarioIds`, `mode`<br>**Process**: Controllerがデータ(User/Place)を取得し、`scoring.ts` で計算を実行。<br><b>自動モード:</b> ベクトル合成 `(Global * 0.3) + (Scenario * 0.7)`<br><b>手動モード:</b> 軸/シーンの加重平均 (Weight=3)<br>**Output**: `PersonalizedScore[]` |
| **L-06** | **Compare Logic** | **Implemented** | `src/server/actions/comparison.ts` (Controller)<br>`src/server/services/scoring.ts` (Logic) | **Trigger**: 比較実行<br>**Input**: `placeIds`, `uid`, `scenarioIds`<br>**Process**: L-03と共通の `scoring.ts` を使用し、選択中のシーンを反映したマッチ度計算を行う。<br>**Output**: `ComparisonResult` JSON |
...
| **L-10** | **Learning Logic** | **Implemented** | `src/server/actions/user.ts`<br>`submitEvaluation` | **Trigger**: Good/Bad評価/シーン選択<br>**Input**: `uid`, `placeId`, `type`, `scenarioIds`<br>**Process**: AI選好軸(Linear)と嗜好ベクトル(EMA)を更新。選択されたシーンのベクトルも更新。<br>**Output**: Void |
