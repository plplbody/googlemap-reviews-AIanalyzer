...
| **U-06** | **Profile View** | **Implemented** | `src/app/profile/page.tsx`<br>`PlaceListItem.tsx` | **Trigger**: `/profile`遷移<br>**Process**: Interaction履歴取得、タブ切り替え、リスト表示。<br>**Output**: マイページ表示。 |
| **U-10** | **Memo Modal** | **Implemented** | `src/components/MemoModal.tsx` | **Trigger**: U-06の鉛筆ボタン<br>**Process**: `<dialog>`で最前面表示。メモ・タグ入力。<br>**Output**: メモ入力ダイアログ。 |
...
| **L-16** | **Update Memo** | **Implemented** | `src/server/actions/user.ts`<br>`updateInteractionMemo` | **Trigger**: U-10 保存ボタン<br>**Input**: `uid`, `placeId`, `memo`, `repeat`<br>**Process**: Interactionのmemo/repeatフィールドを更新。<br>**Output**: Void |
...
| **L-07** | **Toggle Save** | <span style="color:red">Removed</span> | - | **Deprecation**: Heartボタン(L-10)に統合されたため廃止。 |
...
flowchart LR
    ...
    subgraph ACT_2_1 ["ACT-2-1: いいね（ハート）"]
        direction TB
        U_Action["<b>U-xx: アクションボタン</b><br/>(ActionButtons)<br/>- ハートアイコン"]:::ui
        L_Eval["<b>L-10: 学習ロジック</b><br/>(submitEvaluation)<br/>- 保存&高評価"]:::logic
        D_04_S2[("<b>D-04: 行動履歴</b><br/>(Interactions)")]:::db
        
        U_Action -- "アクション: ハート押下" --> L_Eval
        L_Eval -- "サーバー処理: 保存記録" --> D_04_S2
    end

    subgraph ACT_2_2 ["ACT-2-2: プロフィール確認"]
        direction TB
        U_Profile["<b>U-06: プロフィール画面</b><br/>(PlaceListItem)<br/>- 「興味あり」タブ"]:::ui
        L_Fetch["<b>L-08: データ取得</b><br/>(Page/Layout)"]:::logic
        
        D_04_S2 -. "サーバー処理: 履歴Read" .-> L_Fetch
        L_Fetch -- "クライアント処理: リスト表示" --> U_Profile
    end

    subgraph ACT_2_4 ["ACT-2-4: 来店チェック & メモ"]
        direction TB
        C_Toggle["<b>来店トグル</b><br/>(Switch UI)"]:::comp
        C_Pencil["<b>鉛筆アイコン</b><br/>(来店済アイテム)"]:::comp
        U_Memo["<b>U-10: メモモーダル</b><br/>(MemoModal)<br/>- メモ入力<br/>- リピートタグ"]:::ui
        
        L_Status["<b>L-09: ステータス更新</b><br/>(toggleVisited)"]:::logic
        L_Memo["<b>L-16: メモ更新</b><br/>(updateInteractionMemo)"]:::logic
        
        U_Profile -- "アクション: スイッチON" --> C_Toggle
        C_Toggle -- "クライアント処理: 通知" --> L_Status
        L_Status -- "サーバー処理: 来店フラグ更新" --> D_04_S2
        
        L_Status -. "クライアント処理: アイコン表示" .-> C_Pencil
        C_Pencil -- "アクション: クリック" --> U_Memo
        U_Memo -- "アクション: 保存" --> L_Memo
        L_Memo -- "サーバー処理: メモ保存" --> D_04_S2
    end

    S2_Start --> ACT_2_1
    ACT_2_1 --> ACT_2_2
    ACT_2_2 -- "アクション: 来店" --> ACT_2_4
    ACT_2_4 --> S2_End
```
