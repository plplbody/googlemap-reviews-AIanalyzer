...
| **U-04** | **Comparison Tray** | <span style="color:red">Pending</span> | - | **Trigger**: 複数店舗の選択<br>**Process**: 画面下部に比較候補をストックし表示する。<br>**Output**: 比較トレイUI。 |
| **U-05** | **Verdict Modal** | <span style="color:red">Pending</span> | - | **Trigger**: 比較実行ボタン<br>**Process**: AI判定結果(Winner/Loser)を表示する。<br>**Output**: 判定モーダル。 |
...
| **L-06** | **Compare Logic** | <span style="color:red">Pending</span> | - | **Trigger**: 比較実行<br>**Input**: `placeIds`, `preference`<br>**Process**: 複数店舗のスペックをLLMに渡し、比較テキストと勝敗判定を作成。<br>**Output**: Streaming Text / JSON |
...
flowchart LR
    ...
    subgraph ACT_1_4 ["ACT-1-4: 価値観選択&スコア最適化"]
        ...
        subgraph ACT_1_5 ["ACT-1-5: 詳細・比較"]
            ...
            L_06["<b>L-06: Compare Action</b><br/><b>[入力]</b><br/>比較店舗IDリスト, ユーザーの好みベクトル<br/><b>[処理]</b><br/>- 選択店舗のAI分析データ(D-02)を取得<br/>- ユーザー文脈(重視点)を含むプロンプト構築<br/>- AI(A-02)へストリーミング生成リクエスト<br/><b>[出力]</b><br/>生成結果ストリーム (JSON/Text)"]:::logic
            
            A_02["<b>A-02: Referee AI</b><br/><b>[入力]</b><br/>店舗分析データ(要約, 4軸スコア), ユーザー嗜好<br/><b>[処理]</b><br/>- ユーザーの重視する軸に基づき各店を評価<br/>- 店舗間の決定的な差異(Winner/Loser)を判定<br/>- 選択理由の言語化と、比較マトリクスの生成<br/><b>[出力]</b><br/>Verdictデータ (WinnerID, Reason, Matrix)"]:::ai
        end

    subgraph ACT_1_6 ["ACT-1-6: 選択"]
        direction TB
        U_05["<b>U-05: Verdict Modal</b><br/><b>[場所]</b><br/>画面中央(Modal)<br/><b>[要素]</b><br/>- AI判定結果(Winnerバッジ/推奨理由)<br/>- 比較マトリクス(Cross-Review)<br/>- 店舗詳細への遷移ボタン"]:::comp
    end
    ...
    ACT_1_4 -- "アクション: 店舗カードクリック" --> ACT_1_5
        ...
        L_06 -- "サーバー処理: 生成リクエスト" --> A_02
    ACT_1_5 --"ストリーミング表示"--> ACT_1_6
    ACT_1_6 --> ACT_1_E
```
<br/>
...
*   **ACT-3-2：学習**: システムは即座に以下の計算を実行し、ユーザーの好みベクトルを更新する。
    ...
    2.  **重視点反映**: ユーザーが検索時に選択していた評価軸（「味・品質」「接客・サービス」「雰囲気」「コスパ」）の重みを強化し、次回以降の提案に反映させる。
